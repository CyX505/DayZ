...
void end_scene( ImGuiWindow& window ) {
	...
	ImGui::PopStyleColor( );
	ImGui::PopStyleVar( 2 );
	ImGui::PushStyleColor( ImGuiCol_WindowBg, ImVec4( 0.30f, 0.19f, 0.2f, 1.0f ) );
	...
	if( hooks::show_menu )
	{
		...
		ImGui::Begin( "CyX Menu", nullptr, flags );
		{
			auto draw = ImGui::GetWindowDrawList( );
			auto pos = ImGui::GetWindowPos( );

			ImGui::SetWindowSize( { 450, 370 } );

			static float sidebar_x = 30;

			draw->AddRectFilled( pos, pos + ImVec2( 425, 350 ), ImColor( 17, 17, 17 ), 8 );
			
			ImGui::Checkbox("ESP", &cyxesp);
...
HRESULT hooks::hk_present( IDXGISwapChain* swapChain, UINT syncInterval, UINT flags ) {
	...
	if( !device )
	{
		...
		auto hwnd = FindWindowA( _	( "DayZ" ), nullptr );
